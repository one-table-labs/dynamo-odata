...
class ODataLexer(Lexer):
    ...
    @_(rf"{_RWS}not_exists")
    def NOT_EXISTS(self, t):
        ":meta private:"
        t.value = ast.Not_Exists()
        return t
...
class ODataParser(Parser):
    ...
    #
    # NOTE: NOT is placed BELOW EQ/NE (i.e. lower precedence than comparisons).
    # The OData spec places unary 'not' above comparisons, but for DynamoDB filter
    # expressions that produces `(not field) eq value` which is a TypeError at
    # eval time.  Placing NOT below comparisons means `not field eq value` is
    # parsed as `not (field eq value)` which is the intended semantic.
    # UMINUS is kept above arithmetic operators for normal unary-minus behaviour.
    precedence = (
        ("left", "OR"),
        ("left", "AND"),
        ("right", "NOT"),  # logical NOT: lower than comparisons
        ("left", "EQ", "NE"),
        ("left", "GT", "GE", "LT", "LE"),
        ("left", "ADD", "SUB"),
        ("left", "MUL", "DIV", "MOD"),
        ("right", "UMINUS"),  # unary minus: still above arithmetic
        ("left", "IN"),
    )
    ...
    @_(  # type:ignore[no-redef]
        "common_expr EXISTS",
        "common_expr NOT_EXISTS",
    )
    def common_expr(self, p):
        ":meta private:"
        return ast.Function(p[1], p[0])
...
def parse_odata(filter_str: str) -> ast._Node:
    lexer = ODataLexer()
    parser = ODataParser()
    return parser.parse(lexer.tokenize(filter_str))
